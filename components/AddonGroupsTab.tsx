...
import { Plus, Edit2, Trash2, Copy, AlertCircle, X, Check, Link, Search } from 'lucide-react';
...
export default function AddonGroupsTab() {
  ...
  const selectProductForOption = (product: any) => {
    if (pickingIndex === null || !editingGroup || !editingGroup.options) return;
    
    const newOptions = [...editingGroup.options];
    newOptions[pickingIndex] = { 
      ...newOptions[pickingIndex], 
      name: product.name,
      price: product.price.toString(), // Store as string for input
      productId: product.id,
      trackStock: true // default to true when linked to product
    };
  ...
  const unlinkProduct = (index: number) => {
    if (!editingGroup || !editingGroup.options) return;
    const newOptions = [...editingGroup.options];
    newOptions[index] = { ...newOptions[index], productId: undefined, trackStock: false };
    setEditingGroup({ ...editingGroup, options: newOptions });
  };
  ...
  return (
    <div className="space-y-6">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="relative flex-1">
                                  <input 
                                    ...
                                    className={`w-full bg-transparent border border-slate-300 dark:border-slate-700 rounded px-2 py-1.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-slate-900 dark:text-slate-100 ${opt.productId ? 'border-blue-300 dark:border-blue-800 bg-blue-50/10' : ''}`}
                                    placeholder="Ex: Bacon, Ketchup..."
                                  />
                                  {opt.productId && (
                                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                                      <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30 px-1.5 py-0.5 rounded leading-none uppercase">Item Vinculado</span>
                                      <button 
                                        type="button"
                                        onClick={() => unlinkProduct(index)}
                                        className="text-slate-400 hover:text-red-500 transition-colors"
                                        title="Remover vínculo com o produto"
                                      >
                                        <X size={12} />
                                      </button>
                                    </div>
                                ...
                                <button
                                  ...
                                  className={`shrink-0 p-1.5 rounded-lg border transition-all ${opt.productId ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-slate-900 text-slate-400 border-slate-300 dark:border-slate-700 hover:text-blue-600 hover:border-blue-500'}`}
                                  title={opt.productId ? "Alterar produto vinculado" : "Vincular a um produto do cardápio"}
                                >
                                  <Link size={16} />
                                </button>
                              </div>
      ...
      {/* MODAL PARA VINCULAR PRODUTO AO ADICIONAL */}
      {isProductPickerOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
              <div>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">Vincular Produto</h2>
                <p className="text-sm text-slate-500">Selecione um item do cardápio para vincular</p>
              </div>
              <button 
            ...
            <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                />
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 gap-1">
                {products
                  ...
                  .map((product) => (
                    <button
                      key={product.id}
                      onClick={() => selectProductForOption(product)}
                      className="w-full text-left p-3 rounded-2xl hover:bg-blue-50 dark:hover:bg-blue-900/20 active:bg-blue-100 transition-all group flex justify-between items-center"
                    >
                      <div>
                        <div className="font-bold text-slate-800 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400">{product.name}</div>
                        <div className="text-xs text-slate-500 uppercase tracking-tight">{product.category}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-black text-slate-900 dark:text-white">{formatCurrency(product.price)}</div>
                        <div className="text-[10px] text-slate-400">Estoque: {product.stock} {product.unit || 'un'}</div>
                      </div>
                    </button>
                  ))}
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 text-center">
              <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Selecione um item para confirmar o vínculo</p>
            </div>
          </div>
        </div>
      )}
